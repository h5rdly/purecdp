# DO NOT EDIT — generated by purecdp._generator from Audits (devtools-protocol 3bd8f01934b5)
'''CDP domain: Audits.
...
class AffectedRequest:
    ...
    url: str
    #: *(Optional)* The unique request id.
    ...
    def from_json(cls, json: T_JSON_DICT) -> AffectedRequest:
        return cls(
            url=json['url'],
            request_id=network.RequestId.from_json(json['requestId']) if json.get('requestId') is not None else None,
        )
...
class CookieExclusionReason(enum.StrEnum):
...
class CookieWarningReason(enum.StrEnum):
...
class CookieOperation(enum.StrEnum):
...
class CookieIssueInsight:
    ...
    type: InsightType
    #: *(Optional)* Link to table entry in third-party cookie migration readiness list.
    ...
    def from_json(cls, json: T_JSON_DICT) -> CookieIssueInsight:
        return cls(
            type=InsightType.from_json(json['type']),
            table_entry_url=json['tableEntryUrl'] if json.get('tableEntryUrl') is not None else None,
        )
...
class CookieIssueDetails:
    ...
    #: *(Optional)* If AffectedCookie is not set then rawCookieLine contains the raw Set-Cookie header string. This hints at a problem where the cookie line is syntactically or semantically malformed in a way that no valid cookie could be created.
    ...
    #: *(Optional)* The recommended solution to the issue.
    ...
    def from_json(cls, json: T_JSON_DICT) -> CookieIssueDetails:
        return cls(
            ...
            cookie=AffectedCookie.from_json(json['cookie']) if json.get('cookie') is not None else None,
            raw_cookie_line=json['rawCookieLine'] if json.get('rawCookieLine') is not None else None,
            site_for_cookies=json['siteForCookies'] if json.get('siteForCookies') is not None else None,
            cookie_url=json['cookieUrl'] if json.get('cookieUrl') is not None else None,
            request=AffectedRequest.from_json(json['request']) if json.get('request') is not None else None,
            insight=CookieIssueInsight.from_json(json['insight']) if json.get('insight') is not None else None,
        )


class PerformanceIssueType(enum.StrEnum):
...
class PerformanceIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> PerformanceIssueDetails:
        return cls(
            ...
            source_code_location=SourceCodeLocation.from_json(json['sourceCodeLocation']) if json.get('sourceCodeLocation') is not None else None,
        )


class MixedContentResolutionStatus(enum.StrEnum):
...
class MixedContentResourceType(enum.StrEnum):
    AUDIO = 'Audio'
    BEACON = 'Beacon'
    CSPREPORT = 'CSPReport'
    DOWNLOAD = 'Download'
...
class MixedContentIssueDetails:
    ...
    #: *(Optional)* The type of resource causing the mixed content issue (css, js, iframe, form,...). Marked as optional because it is mapped to from blink::mojom::RequestContextType, which will be replaced by network::mojom::RequestDestination
    resource_type: MixedContentResourceType | None = None
    #: *(Optional)* The mixed content request. Does not always exist (e.g. for unsafe form submission urls).
    request: AffectedRequest | None = None
    #: *(Optional)* Optional because not every mixed content issue is necessarily linked to a frame.
    ...
    def from_json(cls, json: T_JSON_DICT) -> MixedContentIssueDetails:
        return cls(
            ...
            insecure_url=json['insecureURL'],
            main_resource_url=json['mainResourceURL'],
            resource_type=MixedContentResourceType.from_json(json['resourceType']) if json.get('resourceType') is not None else None,
            request=AffectedRequest.from_json(json['request']) if json.get('request') is not None else None,
            frame=AffectedFrame.from_json(json['frame']) if json.get('frame') is not None else None,
        )


class BlockedByResponseReason(enum.StrEnum):
    '''Enum indicating the reason a response has been blocked. These reasons are
    refinements of the net error BLOCKED_BY_RESPONSE.
    '''
    COEP_FRAME_RESOURCE_NEEDS_COEP_HEADER = 'CoepFrameResourceNeedsCoepHeader'
    COOP_SANDBOXED_IFRAME_CANNOT_NAVIGATE_TO_COOP_PAGE = 'CoopSandboxedIFrameCannotNavigateToCoopPage'
    ...
    SRIMESSAGE_SIGNATURE_MISMATCH = 'SRIMessageSignatureMismatch'
...
class BlockedByResponseIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> BlockedByResponseIssueDetails:
        return cls(
            ...
            parent_frame=AffectedFrame.from_json(json['parentFrame']) if json.get('parentFrame') is not None else None,
            blocked_frame=AffectedFrame.from_json(json['blockedFrame']) if json.get('blockedFrame') is not None else None,
        )


class HeavyAdResolutionStatus(enum.StrEnum):
...
class HeavyAdReason(enum.StrEnum):
...
class ContentSecurityPolicyViolationType(enum.StrEnum):
    K_INLINE_VIOLATION = 'kInlineViolation'
    K_EVAL_VIOLATION = 'kEvalViolation'
    K_URLVIOLATION = 'kURLViolation'
    K_SRIVIOLATION = 'kSRIViolation'
...
class SourceCodeLocation:
    ...
    def from_json(cls, json: T_JSON_DICT) -> SourceCodeLocation:
        return cls(
            ...
            script_id=runtime.ScriptId.from_json(json['scriptId']) if json.get('scriptId') is not None else None,
        )
...
class ContentSecurityPolicyIssueDetails:
    ...
    #: *(Optional)* The url not included in allowed sources.
    ...
    def from_json(cls, json: T_JSON_DICT) -> ContentSecurityPolicyIssueDetails:
        return cls(
            violated_directive=json['violatedDirective'],
            is_report_only=json['isReportOnly'],
            ...
            blocked_url=json['blockedURL'] if json.get('blockedURL') is not None else None,
            frame_ancestor=AffectedFrame.from_json(json['frameAncestor']) if json.get('frameAncestor') is not None else None,
            source_code_location=SourceCodeLocation.from_json(json['sourceCodeLocation']) if json.get('sourceCodeLocation') is not None else None,
            violating_node_id=dom.BackendNodeId.from_json(json['violatingNodeId']) if json.get('violatingNodeId') is not None else None,
        )


class SharedArrayBufferIssueType(enum.StrEnum):
...
class SharedArrayBufferIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> SharedArrayBufferIssueDetails:
        return cls(
            ...
            is_warning=json['isWarning'],
            type=SharedArrayBufferIssueType.from_json(json['type']),
        )
...
class CorsIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> CorsIssueDetails:
        return cls(
            ...
            is_warning=json['isWarning'],
            request=AffectedRequest.from_json(json['request']),
            location=SourceCodeLocation.from_json(json['location']) if json.get('location') is not None else None,
            initiator_origin=json['initiatorOrigin'] if json.get('initiatorOrigin') is not None else None,
            resource_ip_address_space=network.IPAddressSpace.from_json(json['resourceIPAddressSpace']) if json.get('resourceIPAddressSpace') is not None else None,
            client_security_state=network.ClientSecurityState.from_json(json['clientSecurityState']) if json.get('clientSecurityState') is not None else None,
        )


class SharedDictionaryError(enum.StrEnum):
    ...
    WRITE_ERROR_INVALID_TTLFIELD = 'WriteErrorInvalidTTLField'
    WRITE_ERROR_NAVIGATION_REQUEST = 'WriteErrorNavigationRequest'
    WRITE_ERROR_NO_MATCH_FIELD = 'WriteErrorNoMatchField'
    WRITE_ERROR_NON_INTEGER_TTLFIELD = 'WriteErrorNonIntegerTTLField'
...
class SRIMessageSignatureError(enum.StrEnum):
...
class UnencodedDigestError(enum.StrEnum):
...
class ConnectionAllowlistError(enum.StrEnum):
...
class QuirksModeIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> QuirksModeIssueDetails:
        return cls(
            is_limited_quirks_mode=json['isLimitedQuirksMode'],
...
class SRIMessageSignatureIssueDetails:
    ...
    def to_json(self) -> T_JSON_DICT:
        ...
        json['integrityAssertions'] = list(self.integrity_assertions)
        json['request'] = self.request.to_json()
        return json
    ...
    def from_json(cls, json: T_JSON_DICT) -> SRIMessageSignatureIssueDetails:
        return cls(
            ...
            signature_base=json['signatureBase'],
            integrity_assertions=list(json['integrityAssertions']),
            request=AffectedRequest.from_json(json['request']),
        )
...
class GenericIssueErrorType(enum.StrEnum):
    ...
    BACK_UINAVIGATION_WOULD_SKIP_AD = 'BackUINavigationWouldSkipAd'
...
class GenericIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> GenericIssueDetails:
        return cls(
            ...
            violating_node_id=dom.BackendNodeId.from_json(json['violatingNodeId']) if json.get('violatingNodeId') is not None else None,
            violating_node_attribute=json['violatingNodeAttribute'] if json.get('violatingNodeAttribute') is not None else None,
            request=AffectedRequest.from_json(json['request']) if json.get('request') is not None else None,
        )
...
class DeprecationIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> DeprecationIssueDetails:
        return cls(
            source_code_location=SourceCodeLocation.from_json(json['sourceCodeLocation']),
            type=json['type'],
            affected_frame=AffectedFrame.from_json(json['affectedFrame']) if json.get('affectedFrame') is not None else None,
        )
...
class BounceTrackingIssueDetails:
    ...
    def to_json(self) -> T_JSON_DICT:
        json: T_JSON_DICT = {}
        json['trackingSites'] = list(self.tracking_sites)
        return json

    @classmethod
    def from_json(cls, json: T_JSON_DICT) -> BounceTrackingIssueDetails:
        return cls(
            tracking_sites=list(json['trackingSites']),
        )
...
class CookieDeprecationMetadataIssueDetails:
    ...
    def to_json(self) -> T_JSON_DICT:
        json: T_JSON_DICT = {}
        json['allowedSites'] = list(self.allowed_sites)
    ...
    def from_json(cls, json: T_JSON_DICT) -> CookieDeprecationMetadataIssueDetails:
        return cls(
            allowed_sites=list(json['allowedSites']),
            opt_out_percentage=float(json['optOutPercentage']),
            is_opt_out_top_level=json['isOptOutTopLevel'],
            operation=CookieOperation.from_json(json['operation']),
        )


class ClientHintIssueReason(enum.StrEnum):
...
class FederatedAuthRequestIssueReason(enum.StrEnum):
...
class FederatedAuthUserInfoRequestIssueReason(enum.StrEnum):
...
class EmailVerificationRequestIssueReason(enum.StrEnum):
...
class PartitioningBlobURLInfo(enum.StrEnum):
...
class ElementAccessibilityIssueReason(enum.StrEnum):
...
class ElementAccessibilityIssueDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> ElementAccessibilityIssueDetails:
        return cls(
            ...
            has_disallowed_attributes=json['hasDisallowedAttributes'],
        )


class StyleSheetLoadingIssueReason(enum.StrEnum):
...
class StylesheetLoadingIssueDetails:
    ...
    #: *(Optional)* Contains additional info when the failure was due to a request.
    ...
    def from_json(cls, json: T_JSON_DICT) -> StylesheetLoadingIssueDetails:
        return cls(
            ...
            failed_request_info=FailedRequestInfo.from_json(json['failedRequestInfo']) if json.get('failedRequestInfo') is not None else None,
        )


class PropertyRuleIssueReason(enum.StrEnum):
...
class PropertyRuleIssueDetails:
    ...
    #: *(Optional)* The value of the property rule property that failed to parse
    ...
    def from_json(cls, json: T_JSON_DICT) -> PropertyRuleIssueDetails:
        return cls(
            ...
            property_value=json['propertyValue'] if json.get('propertyValue') is not None else None,
        )


class UserReidentificationIssueType(enum.StrEnum):
...
class UserReidentificationIssueDetails:
    ...
    #: *(Optional)* Applies to BlockedFrameNavigation and BlockedSubresource issue types.
    request: AffectedRequest | None = None
    #: *(Optional)* Applies to NoisedCanvasReadback issue type.
    source_code_location: SourceCodeLocation | None = None
    ...
    def from_json(cls, json: T_JSON_DICT) -> UserReidentificationIssueDetails:
        return cls(
            type=UserReidentificationIssueType.from_json(json['type']),
            request=AffectedRequest.from_json(json['request']) if json.get('request') is not None else None,
            source_code_location=SourceCodeLocation.from_json(json['sourceCodeLocation']) if json.get('sourceCodeLocation') is not None else None,
        )


class PermissionElementIssueType(enum.StrEnum):
...
class PermissionElementIssueDetails:
    ...
    #: *(Optional)* The value of the type attribute.
    type: str | None = None
    #: *(Optional)* The node ID of the <permission> element.
    node_id: dom.BackendNodeId | None = None
    #: *(Optional)* True if the issue is a warning, false if it is an error.
    is_warning: bool | None = None
    #: *(Optional)* Fields for message construction: Used for messages that reference a specific permission name
    permission_name: str | None = None
    #: *(Optional)* Used for messages about occlusion
    occluder_node_info: str | None = None
    #: *(Optional)* Used for messages about occluder's parent
    occluder_parent_node_info: str | None = None
    #: *(Optional)* Used for messages about activation disabled reason
    ...
    def from_json(cls, json: T_JSON_DICT) -> PermissionElementIssueDetails:
        return cls(
            ...
            node_id=dom.BackendNodeId.from_json(json['nodeId']) if json.get('nodeId') is not None else None,
            is_warning=json['isWarning'] if json.get('isWarning') is not None else None,
            permission_name=json['permissionName'] if json.get('permissionName') is not None else None,
            occluder_node_info=json['occluderNodeInfo'] if json.get('occluderNodeInfo') is not None else None,
            occluder_parent_node_info=json['occluderParentNodeInfo'] if json.get('occluderParentNodeInfo') is not None else None,
            disable_reason=json['disableReason'] if json.get('disableReason') is not None else None,
        )
...
class SelectivePermissionsInterventionIssueDetails:
    ...
    #: *(Optional)* The stack trace at the time of the intervention.
...
class InspectorIssueCode(enum.StrEnum):
    ...
    PARTITIONING_BLOB_URLISSUE = 'PartitioningBlobURLIssue'
    ...
    SRIMESSAGE_SIGNATURE_ISSUE = 'SRIMessageSignatureIssue'
...
class InspectorIssueDetails:
    ...
    #: *(Optional)*
    selective_permissions_intervention_issue_details: SelectivePermissionsInterventionIssueDetails | None = None
    #: *(Optional)*
    email_verification_request_issue_details: EmailVerificationRequestIssueDetails | None = None
    #: *(Optional)*
    ...
    def from_json(cls, json: T_JSON_DICT) -> InspectorIssueDetails:
        return cls(
            cookie_issue_details=CookieIssueDetails.from_json(json['cookieIssueDetails']) if json.get('cookieIssueDetails') is not None else None,
            mixed_content_issue_details=MixedContentIssueDetails.from_json(json['mixedContentIssueDetails']) if json.get('mixedContentIssueDetails') is not None else None,
            blocked_by_response_issue_details=BlockedByResponseIssueDetails.from_json(json['blockedByResponseIssueDetails']) if json.get('blockedByResponseIssueDetails') is not None else None,
            heavy_ad_issue_details=HeavyAdIssueDetails.from_json(json['heavyAdIssueDetails']) if json.get('heavyAdIssueDetails') is not None else None,
            content_security_policy_issue_details=ContentSecurityPolicyIssueDetails.from_json(json['contentSecurityPolicyIssueDetails']) if json.get('contentSecurityPolicyIssueDetails') is not None else None,
            shared_array_buffer_issue_details=SharedArrayBufferIssueDetails.from_json(json['sharedArrayBufferIssueDetails']) if json.get('sharedArrayBufferIssueDetails') is not None else None,
            cors_issue_details=CorsIssueDetails.from_json(json['corsIssueDetails']) if json.get('corsIssueDetails') is not None else None,
            quirks_mode_issue_details=QuirksModeIssueDetails.from_json(json['quirksModeIssueDetails']) if json.get('quirksModeIssueDetails') is not None else None,
            partitioning_blob_url_issue_details=PartitioningBlobURLIssueDetails.from_json(json['partitioningBlobURLIssueDetails']) if json.get('partitioningBlobURLIssueDetails') is not None else None,
            navigator_user_agent_issue_details=NavigatorUserAgentIssueDetails.from_json(json['navigatorUserAgentIssueDetails']) if json.get('navigatorUserAgentIssueDetails') is not None else None,
            generic_issue_details=GenericIssueDetails.from_json(json['genericIssueDetails']) if json.get('genericIssueDetails') is not None else None,
            deprecation_issue_details=DeprecationIssueDetails.from_json(json['deprecationIssueDetails']) if json.get('deprecationIssueDetails') is not None else None,
            client_hint_issue_details=ClientHintIssueDetails.from_json(json['clientHintIssueDetails']) if json.get('clientHintIssueDetails') is not None else None,
            federated_auth_request_issue_details=FederatedAuthRequestIssueDetails.from_json(json['federatedAuthRequestIssueDetails']) if json.get('federatedAuthRequestIssueDetails') is not None else None,
            bounce_tracking_issue_details=BounceTrackingIssueDetails.from_json(json['bounceTrackingIssueDetails']) if json.get('bounceTrackingIssueDetails') is not None else None,
            cookie_deprecation_metadata_issue_details=CookieDeprecationMetadataIssueDetails.from_json(json['cookieDeprecationMetadataIssueDetails']) if json.get('cookieDeprecationMetadataIssueDetails') is not None else None,
            stylesheet_loading_issue_details=StylesheetLoadingIssueDetails.from_json(json['stylesheetLoadingIssueDetails']) if json.get('stylesheetLoadingIssueDetails') is not None else None,
            property_rule_issue_details=PropertyRuleIssueDetails.from_json(json['propertyRuleIssueDetails']) if json.get('propertyRuleIssueDetails') is not None else None,
            federated_auth_user_info_request_issue_details=FederatedAuthUserInfoRequestIssueDetails.from_json(json['federatedAuthUserInfoRequestIssueDetails']) if json.get('federatedAuthUserInfoRequestIssueDetails') is not None else None,
            shared_dictionary_issue_details=SharedDictionaryIssueDetails.from_json(json['sharedDictionaryIssueDetails']) if json.get('sharedDictionaryIssueDetails') is not None else None,
            element_accessibility_issue_details=ElementAccessibilityIssueDetails.from_json(json['elementAccessibilityIssueDetails']) if json.get('elementAccessibilityIssueDetails') is not None else None,
            sri_message_signature_issue_details=SRIMessageSignatureIssueDetails.from_json(json['sriMessageSignatureIssueDetails']) if json.get('sriMessageSignatureIssueDetails') is not None else None,
            unencoded_digest_issue_details=UnencodedDigestIssueDetails.from_json(json['unencodedDigestIssueDetails']) if json.get('unencodedDigestIssueDetails') is not None else None,
            connection_allowlist_issue_details=ConnectionAllowlistIssueDetails.from_json(json['connectionAllowlistIssueDetails']) if json.get('connectionAllowlistIssueDetails') is not None else None,
            user_reidentification_issue_details=UserReidentificationIssueDetails.from_json(json['userReidentificationIssueDetails']) if json.get('userReidentificationIssueDetails') is not None else None,
            permission_element_issue_details=PermissionElementIssueDetails.from_json(json['permissionElementIssueDetails']) if json.get('permissionElementIssueDetails') is not None else None,
            performance_issue_details=PerformanceIssueDetails.from_json(json['performanceIssueDetails']) if json.get('performanceIssueDetails') is not None else None,
            selective_permissions_intervention_issue_details=SelectivePermissionsInterventionIssueDetails.from_json(json['selectivePermissionsInterventionIssueDetails']) if json.get('selectivePermissionsInterventionIssueDetails') is not None else None,
            email_verification_request_issue_details=EmailVerificationRequestIssueDetails.from_json(json['emailVerificationRequestIssueDetails']) if json.get('emailVerificationRequestIssueDetails') is not None else None,
            lazy_load_image_issue_details=LazyLoadImageIssueDetails.from_json(json['lazyLoadImageIssueDetails']) if json.get('lazyLoadImageIssueDetails') is not None else None,
        )
...
class IssueId(str):
    ...
    def __repr__(self) -> str:
        return f'IssueId({str.__repr__(self)})'
...
class InspectorIssue:
    ...
    #: *(Optional)* A unique id for this issue. May be omitted if no other entity (e.g. exception, CDP message, etc.) is referencing this issue.
    ...
    def from_json(cls, json: T_JSON_DICT) -> InspectorIssue:
        return cls(
            ...
            issue_id=IssueId.from_json(json['issueId']) if json.get('issueId') is not None else None,
        )
...
def get_encoded_response(
    ...
    :param encoding: The encoding to use. Allowed values: webp, jpeg, png
    ...
    json = yield {'method': 'Audits.getEncodedResponse', 'params': params}
    return (
        json['body'] if json.get('body') is not None else None,
        json['originalSize'],
        json['encodedSize'],
    )
...
def check_forms_issues() -> typing.Generator[T_JSON_DICT, T_JSON_DICT, list[GenericIssueDetails]]:
    ...
    :returns:
    '''
    json = yield {'method': 'Audits.checkFormsIssues'}
