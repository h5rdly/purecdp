# DO NOT EDIT — generated by purecdp._generator from Preload (devtools-protocol 3bd8f01934b5)
'''CDP domain: Preload.
...
class RuleSetId(str):
    ...
    def __repr__(self) -> str:
        return f'RuleSetId({str.__repr__(self)})'
...
class RuleSet:
    ...
    source_text: str
    #: *(Optional)* A speculation rule set is either added through an inline `<script>` tag or through an external resource via the 'Speculation-Rules' HTTP header. For the first case, we include the BackendNodeId of the relevant `<script>` tag. For the second case, we include the external URL where the rule set was loaded from, and also RequestId if Network domain is enabled.  See also: - https://wicg.github.io/nav-speculation/speculation-rules.html#speculation-rules-script - https://wicg.github.io/nav-speculation/speculation-rules.html#speculation-rules-header
    backend_node_id: dom.BackendNodeId | None = None
    #: *(Optional)*
    url: str | None = None
    #: *(Optional)*
    request_id: network.RequestId | None = None
    #: *(Optional)* Error information `errorMessage` is null iff `errorType` is null.
    error_type: RuleSetErrorType | None = None
    #: *(Optional)* TODO(https://crbug.com/1425354): Replace this property with structured error. *(DEPRECATED)*
    error_message: str | None = None
    #: *(Optional)* For more details, see: https://github.com/WICG/nav-speculation/blob/main/speculation-rules-tags.md
    tag: str | None = None
    ...
    def from_json(cls, json: T_JSON_DICT) -> RuleSet:
        return cls(
            id=RuleSetId.from_json(json['id']),
            loader_id=network.LoaderId.from_json(json['loaderId']),
            source_text=json['sourceText'],
            ...
            error_type=RuleSetErrorType.from_json(json['errorType']) if json.get('errorType') is not None else None,
            error_message=json['errorMessage'] if json.get('errorMessage') is not None else None,
            tag=json['tag'] if json.get('tag') is not None else None,
        )


class RuleSetErrorType(enum.StrEnum):
...
class SpeculationAction(enum.StrEnum):
...
class SpeculationTargetHint(enum.StrEnum):
...
class PreloadingAttemptKey:
    ...
    def from_json(cls, json: T_JSON_DICT) -> PreloadingAttemptKey:
        return cls(
            ...
            url=json['url'],
            form_submission=json['formSubmission'] if json.get('formSubmission') is not None else None,
            target_hint=SpeculationTargetHint.from_json(json['targetHint']) if json.get('targetHint') is not None else None,
        )
...
class PreloadPipelineId(str):
    ...
    def __repr__(self) -> str:
        return f'PreloadPipelineId({str.__repr__(self)})'


class PrerenderFinalStatus(enum.StrEnum):
...
class PreloadingStatus(enum.StrEnum):
...
class PrefetchStatus(enum.StrEnum):
    ...
    PREFETCH_FAILED_MIMENOT_SUPPORTED = 'PrefetchFailedMIMENotSupported'
...
class PrerenderMismatchedHeaders:
    ...
    def from_json(cls, json: T_JSON_DICT) -> PrerenderMismatchedHeaders:
        return cls(
            header_name=json['headerName'],
            initial_value=json['initialValue'] if json.get('initialValue') is not None else None,
            activation_value=json['activationValue'] if json.get('activationValue') is not None else None,
        )


def enable() -> typing.Generator[T_JSON_DICT, T_JSON_DICT, None]:
    yield {'method': 'Preload.enable'}


def disable() -> typing.Generator[T_JSON_DICT, T_JSON_DICT, None]:
    yield {'method': 'Preload.disable'}
...
class PreloadEnabledStateUpdated:
    ...
    def from_json(cls, json: T_JSON_DICT) -> PreloadEnabledStateUpdated:
        return cls(
            disabled_by_preference=json['disabledByPreference'],
            disabled_by_data_saver=json['disabledByDataSaver'],
            disabled_by_battery_saver=json['disabledByBatterySaver'],
            disabled_by_holdback_prefetch_speculation_rules=json['disabledByHoldbackPrefetchSpeculationRules'],
            disabled_by_holdback_prerender_speculation_rules=json['disabledByHoldbackPrerenderSpeculationRules'],
        )
...
class PrerenderStatusUpdated:
    ...
    #: *(Optional)* This is used to give users more information about the name of Mojo interface that is incompatible with prerender and has caused the cancellation of the attempt.
    ...
    def from_json(cls, json: T_JSON_DICT) -> PrerenderStatusUpdated:
        return cls(
            ...
            pipeline_id=PreloadPipelineId.from_json(json['pipelineId']),
            status=PreloadingStatus.from_json(json['status']),
            prerender_status=PrerenderFinalStatus.from_json(json['prerenderStatus']) if json.get('prerenderStatus') is not None else None,
            disallowed_mojo_interface=json['disallowedMojoInterface'] if json.get('disallowedMojoInterface') is not None else None,
            mismatched_headers=[PrerenderMismatchedHeaders.from_json(i) for i in json['mismatchedHeaders']] if json.get('mismatchedHeaders') is not None else None,
        )
