# DO NOT EDIT — generated by purecdp._generator from Input (devtools-protocol 3bd8f01934b5)
'''CDP domain: Input.'''
...
class TouchPoint:
    ...
    #: *(Optional)* The normalized tangential pressure, which has a range of [-1,1] (default: 0). *(EXPERIMENTAL)*
    ...
    #: *(Optional)* The clockwise rotation of a pen stylus around its own major axis, in degrees in the range [0,359] (default: 0). *(EXPERIMENTAL)*
    twist: int | None = None
    ...
    def from_json(cls, json: T_JSON_DICT) -> TouchPoint:
        return cls(
            x=float(json['x']),
            y=float(json['y']),
            radius_x=float(json['radiusX']) if json.get('radiusX') is not None else None,
            radius_y=float(json['radiusY']) if json.get('radiusY') is not None else None,
            rotation_angle=float(json['rotationAngle']) if json.get('rotationAngle') is not None else None,
            force=float(json['force']) if json.get('force') is not None else None,
            tangential_pressure=float(json['tangentialPressure']) if json.get('tangentialPressure') is not None else None,
            tilt_x=float(json['tiltX']) if json.get('tiltX') is not None else None,
            tilt_y=float(json['tiltY']) if json.get('tiltY') is not None else None,
            twist=json['twist'] if json.get('twist') is not None else None,
            id=float(json['id']) if json.get('id') is not None else None,
        )


class GestureSourceType(enum.StrEnum):
    '''**EXPERIMENTAL**'''
    DEFAULT = 'default'
    TOUCH = 'touch'
    MOUSE = 'mouse'
...
class DragDataItem:
    ...
    data: str
    #: *(Optional)* Title associated with a link. Only valid when `mimeType` == "text/uri-list".
    title: str | None = None
    #: *(Optional)* Stores the base URL for the contained markup. Only valid when `mimeType` == "text/html".
...
class DragData:
    ...
    #: *(Optional)* List of filenames that should be included when dropping
    ...
    def from_json(cls, json: T_JSON_DICT) -> DragData:
        return cls(
            ...
            drag_operations_mask=json['dragOperationsMask'],
            files=list(json['files']) if json.get('files') is not None else None,
        )
...
def dispatch_drag_event(
    ...
    **EXPERIMENTAL**
    ...
    :param type: Type of the drag event. Allowed values: dragEnter, dragOver, drop, dragCancel
    ...
    yield {'method': 'Input.dispatchDragEvent', 'params': params}
...
def dispatch_key_event(
    ...
    :param type: Type of the key event. Allowed values: keyDown, keyUp, rawKeyDown, char
    :param modifiers: *(Optional)* Bit field representing pressed modifier keys. Alt=1, Ctrl=2, Meta/Command=4, Shift=8 (default: 0).
    ...
    yield {'method': 'Input.dispatchKeyEvent', 'params': params}
...
def insert_text(text: str) -> typing.Generator[T_JSON_DICT, T_JSON_DICT, None]:
    ...
    **EXPERIMENTAL**
    ...
    params: T_JSON_DICT = {}
    params['text'] = text
    yield {'method': 'Input.insertText', 'params': params}
...
def ime_set_composition(
    ...
    **EXPERIMENTAL**
    ...
    yield {'method': 'Input.imeSetComposition', 'params': params}
...
def dispatch_mouse_event(
    ...
    :param type: Type of the mouse event. Allowed values: mousePressed, mouseReleased, mouseMoved, mouseWheel
    :param x: X coordinate of the event relative to the main frame's viewport in CSS pixels.
    :param y: Y coordinate of the event relative to the main frame's viewport in CSS pixels. 0 refers to the top of the viewport and Y increases as it proceeds towards the bottom of the viewport.
    :param modifiers: *(Optional)* Bit field representing pressed modifier keys. Alt=1, Ctrl=2, Meta/Command=4, Shift=8 (default: 0).
    :param timestamp: *(Optional)* Time at which the event occurred.
    ...
    :param force: *(Optional)* The normalized pressure, which has a range of [0,1] (default: 0). *(EXPERIMENTAL)*
    :param tangential_pressure: *(Optional)* The normalized tangential pressure, which has a range of [-1,1] (default: 0). *(EXPERIMENTAL)*
    ...
    :param twist: *(Optional)* The clockwise rotation of a pen stylus around its own major axis, in degrees in the range [0,359] (default: 0). *(EXPERIMENTAL)*
    ...
    :param pointer_type: *(Optional)* Pointer type (default: "mouse"). Allowed values: mouse, pen
    '''
    params: T_JSON_DICT = {}
    params['type'] = type
    params['x'] = x
    ...
    yield {'method': 'Input.dispatchMouseEvent', 'params': params}
...
def dispatch_touch_event(
    ...
    :param type: Type of the touch event. TouchEnd and TouchCancel must not contain any touch points, while TouchStart and TouchMove must contains at least one. Allowed values: touchStart, touchEnd, touchMove, touchCancel
    ...
    if timestamp is not None:
        params['timestamp'] = timestamp.to_json()
    yield {'method': 'Input.dispatchTouchEvent', 'params': params}


def cancel_dragging() -> typing.Generator[T_JSON_DICT, T_JSON_DICT, None]:
    '''Cancels any active dragging in the page.'''
    yield {'method': 'Input.cancelDragging'}
...
def emulate_touch_from_mouse_event(
    ...
    **EXPERIMENTAL**
    ...
    if click_count is not None:
        params['clickCount'] = click_count
    yield {'method': 'Input.emulateTouchFromMouseEvent', 'params': params}
...
def set_ignore_input_events(ignore: bool) -> typing.Generator[T_JSON_DICT, T_JSON_DICT, None]:
    ...
    yield {'method': 'Input.setIgnoreInputEvents', 'params': params}
...
def set_intercept_drags(enabled: bool) -> typing.Generator[T_JSON_DICT, T_JSON_DICT, None]:
    ...
    **EXPERIMENTAL**
    ...
    yield {'method': 'Input.setInterceptDrags', 'params': params}
...
def synthesize_pinch_gesture(
    ...
    **EXPERIMENTAL**
    ...
    yield {'method': 'Input.synthesizePinchGesture', 'params': params}
...
def synthesize_scroll_gesture(
    ...
    **EXPERIMENTAL**
    ...
    yield {'method': 'Input.synthesizeScrollGesture', 'params': params}
...
def synthesize_tap_gesture(
    ...
    **EXPERIMENTAL**
    ...
    if gesture_source_type is not None:
        params['gestureSourceType'] = gesture_source_type.to_json()
    yield {'method': 'Input.synthesizeTapGesture', 'params': params}
